...
Required:
  FMP_API_KEY                 Financial Modeling Prep quote/history/profile/news

Optional:
  ALPHA_VANTAGE_API_KEY       Alpha Vantage NEWS_SENTIMENT overlay
  TRADE_TICKERS               Comma-separated ticker universe
...
DEFAULT_TICKERS = ["SOFI", "PLTR", "HOOD", "RIVN", "F", "BAC", "INTC", "SNAP", "UBER", "AMD"]
...
def main() -> None:
    if not os.environ.get("FMP_API_KEY"):
        raise SystemExit("FMP_API_KEY is required. Add it as a GitHub Actions secret and local .env value.")
    ...
    for symbol in universe:
        try:
            picks.append(build_pick(symbol))
        except Exception as exc:  # noqa: BLE001
            errors.append(f"{symbol}: {exc}")
            print(errors[-1], file=sys.stderr)

    picks = sorted(picks, key=lambda item: item["overallScore"], reverse=True)[:5]
    ...
    snapshot = {
        ...
        "providerNote": "FMP quote/history/profile/news" + (" + Alpha Vantage NEWS_SENTIMENT" if os.environ.get("ALPHA_VANTAGE_API_KEY") else ""),
        "universe": universe,
        "picks": picks,
        "errors": errors,
    }
    OUTPUT.write_text(json.dumps(snapshot, indent=2), encoding="utf-8")
    print(f"Wrote {OUTPUT.relative_to(ROOT)} with {len(picks)} picks")
